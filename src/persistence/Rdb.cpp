...
void RdbEncoder::writeLen(std::ofstream& out, uint64_t len) {
    if (len < (1 << 6)) {
        ...
        const uint8_t first = 0x40 | static_cast<uint8_t>((len >> 8) & 0x3F);
        const uint8_t second = static_cast<uint8_t>(len & 0xFF);
        out.write(reinterpret_cast<const char*>(&first), 1);
        out.write(reinterpret_cast<const char*>(&second), 1);
    } else {
        // 简化：暂不支持超大字符串（>16KB）
...
void RdbEncoder::writeKeyValuePair(std::ofstream& out, const std::string& key, RedisObject* obj) {
    if (obj->type() == ObjectType::STRING) {
        ...
        out.put(RDB_TYPE_LIST);
        writeString(out, key);
        auto list_obj = static_cast<ListObject*>(obj);
        const auto values = list_obj->values();
        writeLen(out, values.size());
        for (const auto& value : values) {
            writeString(out, value);
        }
    } else if (obj->type() == ObjectType::SET) {
        out.put(RDB_TYPE_SET);
        writeString(out, key);
        auto set_obj = static_cast<SetObject*>(obj);
        const auto members = set_obj->members();
        writeLen(out, members.size());
        for (const auto& member : members) {
            writeString(out, member);
        }
    } else if (obj->type() == ObjectType::ZSET) {
        out.put(RDB_TYPE_ZSET);
        writeString(out, key);
        auto zset_obj = static_cast<ZSetObject*>(obj);
        const auto entries = zset_obj->members_with_scores();
        writeLen(out, entries.size());
        for (const auto& [member, score] : entries) {
            writeString(out, member);
            writeString(out, ZSetObject::format_score(score));
        }
    } else {
...
uint64_t RdbDecoder::readLen() {
    ...
    if ((byte & 0xC0) == 0) {
        ...
        uint8_t second;
        readExact(reinterpret_cast<char*>(&second), 1);
        return (static_cast<uint64_t>(byte & 0x3F) << 8) | second;
    } else {
...
std::unordered_map<std::string, std::shared_ptr<RedisObject>> RdbDecoder::readKeyValues() {
    ...
    while (true) {
        ...
        if (type == RDB_TYPE_STRING) {
            ...
            const uint64_t value_count = readLen();
            auto list_obj = std::make_shared<ListObject>();
            for (uint64_t i = 0; i < value_count; ++i) {
                list_obj->push_back(readString());
            }
            obj = list_obj;
        } else if (type == RDB_TYPE_SET) {
            const uint64_t member_count = readLen();
            auto set_obj = std::make_shared<SetObject>();
            for (uint64_t i = 0; i < member_count; ++i) {
                set_obj->add(readString());
            }
            obj = set_obj;
        } else if (type == RDB_TYPE_ZSET) {
            const uint64_t member_count = readLen();
            auto zset_obj = std::make_shared<ZSetObject>();
            for (uint64_t i = 0; i < member_count; ++i) {
                const std::string member = readString();
                const std::string score_string = readString();
                char* end = nullptr;
                errno = 0;
                const double score = std::strtod(score_string.c_str(), &end);
                if (errno == ERANGE || end != score_string.c_str() + score_string.size() ||
                    !std::isfinite(score)) {
                    throw std::runtime_error("Invalid ZSET score in RDB");
                }
                zset_obj->add(score, member);
            }
            obj = zset_obj;
        } else {
